...
{## ----schedules/tempCurves--------------------------------------------------------------------------------------
  ...
  timesMin <- seq(0,24,10/60)
  png(file="../plots/tempCurve.png",
      width=1000, height=1500/2, pointsize = 24)
  plot(timesMin, tempCurveAir(timesMin), type="l", 
       ylab = "", xlab="hour of the day",
       main = "Daily Ambient Temperature Curve", ylim = c(10, 25), col="red")
  ...
  tempdata <- as.data.frame(cbind(
    timesMin, 
    tempAir = tempCurveAir(timesMin),
    tempSoil = tempCurveSoil(timesMin)
  ))
...
{
  version <- "microBactTempVersion1.4"
