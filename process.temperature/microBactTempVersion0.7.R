...
{ 
  ...
  names(parameters) <- c("g", "n_taps", "r_taps", "r_tank", "z", "Q_in", 
                         "V_Bottle", "h_0", "V_0", "c_in", "k_20", "Q_10", "T_20", "T_c") 
  ...
  times <- seq(from = 0, to = 60*60*24*7, by = 1) 
  #times <- seq(from = 0, to = 10, by = 1) 
  # remove unused tmp variables
  rm(g, n_taps, r_taps, r_tank, z, Q_in, V_Bottle, h_0) 
}
