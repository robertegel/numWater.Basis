...
outMicro <- microRun(parameters = microStart$parameters, yini = microStart$yini, days = days)
...
if (min(outMicro[[, "h"]]) < h_0) {
  microStart <- defineMicroParametersYini(h_0 = outMicro[[, "h"]])
  outMicro <- microRun(parameters = microStart$parameters, yini = microStart$yini, days = days)
  outMicro[[nrow(outMicro), "V_out"]]
}
...
## -plots micro---------------------------------------------------------------------------------------------------
{
  ...
  plot(simDays, data[, "Q_out"], type = "l", ylab = "Q_out", xlab = "simulation time (hours)", ylim = c(0, max(outMicro[, "Q_out"])))
  dev.off()
