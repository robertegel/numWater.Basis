...
version <- "allModelsVersion1.7.variation.A_roof"
...
variationsA_roof <- c(100, 200, 500, 1000, 2000)

microOutData <- data.frame()
macroOutData <- data.frame()

for (A_roof in variationsA_roof) {
  microStart <- defineMicroParametersYiniVariation(A_roof = A_roof)
  print(paste0("A_roof = ", A_roof))
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"]), A_roof = A_roof)
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles, group=A_roof)) +
      geom_line(size = 0.5, aes(colour = A_roof)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=A_roof)) +
      geom_line(size = 0.5, aes(colour = A_roof)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=A_roof)) +
      geom_line(size = 0.5, aes(colour = A_roof)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=A_roof)) +
      geom_line(size = 0.5, aes(colour = A_roof)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
