...
version <- "allModelsVersion1.7.variation.n_taps"
...
variationsN_taps <- c(1, 3, 8, 10, 20)

microOutData <- data.frame()
macroOutData <- data.frame()

for (n_taps in variationsN_taps) {
  microStart <- defineMicroParametersYiniVariation(n_taps = n_taps)
  print(paste0("n_taps = ", n_taps))
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"]), n_taps = n_taps)
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles,  group=n_taps)) +
      geom_line(size = 0.5, aes(colour = n_taps)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=n_taps)) +
      geom_line(size = 0.5, aes(colour = n_taps)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=n_taps)) +
      geom_line(size = 0.5, aes(colour = n_taps)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=n_taps)) +
      geom_line(size = 0.5, aes(colour = n_taps)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
