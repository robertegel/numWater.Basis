...
version <- "allModelsVersion1.7.variation.h_air.concrete"
...
for (h_air in variationsh_air) {
  microStart <- defineMicroParametersYiniVariation(h_air = h_air)
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"], h_air = h_air))
