...
version <- "allModelsVersion1.7.variation.T_water_0"
...
variationsT_water_0 <- c(10, 15, 17, 20, 30)

microOutData <- data.frame()
macroOutData <- data.frame()

for (T_water_0 in variationsT_water_0) {
  microStart <- defineMicroParametersYiniVariation(T_water_0 = T_water_0)
  print(paste0("T_water_0 = ", T_water_0))
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"]), T_water_0 = T_water_0)
  ...
  tmp$T_water_0 <- as.factor(c(T_water_0))
  tmp$simDays <- simDays
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles,  group=T_water_0)) +
      geom_line(size = 0.5, aes(colour = T_water_0)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=T_water_0)) +
      geom_line(size = 0.5, aes(colour = T_water_0)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=T_water_0)) +
      geom_line(size = 0.5, aes(colour = T_water_0)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=T_water_0)) +
      geom_line(size = 0.5, aes(colour = T_water_0)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
