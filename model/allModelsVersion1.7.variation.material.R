...
version <- "allModelsVersion1.7.variation.material"
...
for (material in variationsmaterial) {
  microStart <- defineMicroParametersYiniVariation(material = material)
  print(paste0("material = ", material))
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"]), material = material)
  ...
  tmp$material <- as.factor(c(material))
  tmp$simDays <- simDays
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles,  group=material)) +
      geom_line(size = 0.5, aes(colour = material)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=material)) +
      geom_line(size = 0.5, aes(colour = material)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=material)) +
      geom_line(size = 0.5, aes(colour = material)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=material)) +
      geom_line(size = 0.5, aes(colour = material)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
