...
version <- "allModelsVersion1.7.variation.c_0"
...
variationsc_0 <- c(1.55 * 10^-5, 1.55 * 10^-4, 1.55 * 10^-4*2, 1.55 * 10^-4*5, 1.55 * 10^-3)

microOutData <- data.frame()
macroOutData <- data.frame()

for (c_0 in variationsc_0) {
  microStart <- defineMicroParametersYiniVariation(c_0 = c_0)
  print(paste0("c_0 = ", c_0))
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"]), c_0 = c_0)
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles,  group=c_0)) +
      geom_line(size = 0.5, aes(colour = c_0)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=c_0)) +
      geom_line(size = 0.5, aes(colour = c_0)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=c_0)) +
      geom_line(size = 0.5, aes(colour = c_0)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=c_0)) +
      geom_line(size = 0.5, aes(colour = c_0)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
