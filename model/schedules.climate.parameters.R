...
tempCurveAir <- function(simHours) {
  ...
  T <- abs(mean(minT) - mean(maxT))/2 * sin((simHours-10) * pi/12) + mean(c(minT, maxT))
  return(T)
}
...
rainCurve <- function(simDay) {
  ...
  pdSpline <- spline(months, pd, n = 365)
...
defineMicroParametersYini <- function(h_0 = 2.5, A_roof = 500){
  basicParameters <- c(
    # I/O micro
    r_tank <- 3, # [m]
    h_0 <- h_0,
...
defineMicroParametersYiniVariation <- function(h_0 = 2.5, A_roof = 500, h_tank = 5, r_tank = 3,
    material = "concrete", c_0 = 1.55 * 10^-4, T_water_0 = 17.0, h_air = 2.5, n_taps = 8, z = 2){
  basicParameters <- c(
    # I/O micro
    r_tank <- r_tank, # [m]
    h_0 <- h_0,
  ...
  parameters <- c(basicParameters, parametersMaterial,
                  # I/O micro
                  g = 9.81, # [m/s^2]
                  n_taps = n_taps,
                  r_taps = 1/1000 * 1, # [m]
...
defineMacroParametersYini <- function(Q_out, h_0 = 2.5, A_roof = 500, h_tank = 5.0, r_tank = 3.0){
  parametersIOmacro <- c(
   radius  <- r_tank,        # [m]
   height  <- h_tank,        # [m]
   h_0     <- h_0          # [m]
  ...
  names(parametersIOmacro) <-c("radius", "height", "h_0")

  parameters <- c(parametersIOmacro,
                  A_roof  = A_roof,     # [m^2]
                  Q_out   = Q_out,     # [m^3/day]
                  cap     = eval(radius^2*pi*height)   # [m^3]
  )

  yini    <- c(
    # I/O macro
    V = eval(h_0*radius^2*pi) # [m^3]
  )
...
interactionPlot <- function(x, y1, y2, nameX, nameY1, nameY2, legendPosition){
  plot(x, y1, type="l", ylab="", xlab = "simulation time (hours)")
  par(new = TRUE)
...
interactionPlot <- function(dataset, nameX, nameY1, nameY2, legendPosition){
  plot(dataset[[nameX]], dataset[[nameY1]], type="l", ylab="", xlab = "simulation time (hours)")
  par(new = TRUE)
