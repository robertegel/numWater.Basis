...
version <- "allModelsVersion1.7.variation.r_tank"
...
variationsr_tank <- c(1, 2, 3, 4, 5)

microOutData <- data.frame()
macroOutData <- data.frame()

for (r_tank in variationsr_tank) {
  microStart <- defineMicroParametersYiniVariation(r_tank = r_tank)
  print(paste0("r_tank = ", r_tank))

  time <- system.time(outMicro <- microRun(parameters = microStart$parameters, yini = microStart$yini, days = 1))
  print(time)
  rm(time)

  macroStart <- defineMacroParametersYini(Q_out = outMicro[[nrow(outMicro), "V_out"]], r_tank = r_tank)
  outMacro <- macroRun(parameters = macroStart$parameters, yini = macroStart$yini)

  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"]), r_tank = r_tank)
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles,  group=r_tank)) +
      geom_line(size = 0.5, aes(colour = r_tank)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=r_tank)) +
      geom_line(size = 0.5, aes(colour = r_tank)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=r_tank)) +
      geom_line(size = 0.5, aes(colour = r_tank)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=r_tank)) +
      geom_line(size = 0.5, aes(colour = r_tank)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
