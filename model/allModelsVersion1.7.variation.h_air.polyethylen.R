...
version <- "allModelsVersion1.7.variation.h_air.polyethylen"
...
variationsh_air <- c(0, 1, 2.5, 3, 5)

microOutData <- data.frame()
macroOutData <- data.frame()

for (h_air in variationsh_air) {
  microStart <- defineMicroParametersYiniVariation(h_air = h_air, material= "polyethylen")
  print(paste0("h_air = ", h_air))
  ...
  if (min(outMacro[, "h"]) < microStart$parameters["h_0"]) {
    microStart <- defineMicroParametersYiniVariation(h_0 = min(outMacro[, "h"], h_air = h_air, material= "polyethylen"))
...
## -plots---------------------------------------------------------------------------------------------------
{
  ...
  plotgird <- plot_grid(
    ggplot(microOutData, aes(simDays, n_Bottles,  group=h_air)) +
      geom_line(size = 0.5, aes(colour = h_air)) +
      xlab("simulation time in days"),
    ggplot(macroOutData, aes(time, V, group=h_air)) +
      geom_line(size = 0.5, aes(colour = h_air)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, T_water, group=h_air)) +
      geom_line(size = 0.5, aes(colour = h_air)) +
      xlab("simulation time in days"),
    ggplot(microOutData, aes(simDays, c, group=h_air)) +
      geom_line(size = 0.5, aes(colour = h_air)) +
      xlab("simulation time in days")
    )
  title <- ggdraw() +
